...
<aside class="w-[240px] min-w-[240px] h-screen fixed top-0 left-0 flex flex-col z-50">
    <a href="#" onclick="navigate('dashboard'); return false;" class="px-5 h-16 flex items-center gap-2.5 no-underline" style="border-bottom: 1px solid var(--border);">
        ...
        <div class="ml-auto text-[9px] font-black rounded px-1.5 py-0.5 tracking-wide" style="background:#e8f5ee;color:#0c7a3e;">ADMIN</div>
    </a>
    ...
    <div class="flex-1 overflow-y-auto py-2">
        ...
        <a href="#" onclick="navigate('add-product'); return false;" class="nav-item" id="nav-add-product"><span class="text-[17px] w-5 text-center">➕</span> Add Product</a>
        <a href="#" onclick="navigate('categories'); return false;" class="nav-item" id="nav-categories"><span class="text-[17px] w-5 text-center">🏷️</span> Categories</a>
        <a href="#" onclick="navigate('banner'); return false;" class="nav-item" id="nav-banner"><span class="text-[17px] w-5 text-center">🖼️</span> Banner & Featured</a>
        <a href="#" onclick="navigate('discounts'); return false;" class="nav-item" id="nav-discounts"><span class="text-[17px] w-5 text-center">🎟️</span> Discounts</a>

        <div class="px-4 pt-4 pb-1.5 text-[10px] font-black text-secondary tracking-widest uppercase">Settings</div>
        <a href="#" onclick="navigate('settings'); return false;" class="nav-item" id="nav-settings"><span class="text-[17px] w-5 text-center">⚙️</span> Store Settings</a>
        <a href="#" onclick="navigate('delivery'); return false;" class="nav-item" id="nav-delivery"><span class="text-[17px] w-5 text-center">🚚</span> Delivery Zones</a>
    </div>
...
<div class="ml-[240px] flex-1 flex flex-col min-h-screen">
    ...
    <div class="topbar h-16 px-8 flex items-center justify-between sticky top-0 z-40">
        ...
        <div class="flex items-center gap-3">
            <button onclick="toggleDarkMode()" id="dmBtn" class="rounded-xl px-3.5 py-2 text-[13px] font-bold cursor-pointer font-nunito flex items-center gap-1.5 transition-colors border-theme border" style="background:var(--input-bg);color:var(--text2);">🌙 Dark</button>
            <a href="/" class="rounded-xl px-3.5 py-2 text-[13px] font-bold cursor-pointer font-nunito flex items-center gap-1.5 no-underline transition-colors border-theme border" style="background:var(--input-bg);color:var(--text2);">🛍️ View Store</a>
            <button class="rounded-xl px-3.5 py-2 text-[13px] font-bold cursor-pointer font-nunito border-theme border" style="background:var(--input-bg);color:var(--text2);">🔔</button>
        </div>
    </div>
    ...
    <div id="page-dashboard" class="page-section p-8">
        <div class="mb-7">
            ...
            <div class="text-[13px] text-secondary font-semibold mt-1">Welcome back, Admin! Here's your store overview.</div>
        </div>
    ...
    <div id="page-products" class="page-section p-8">
        <div class="flex items-center justify-between mb-7">
            <div>
                ...
                <div class="text-[13px] text-secondary font-semibold mt-1">Manage your store's product catalog</div>
            </div>
            <button onclick="navigate('add-product')" class="btn-primary flex items-center gap-2">➕ Add New Product</button>
        </div>
    ...
    <div id="page-customers" class="page-section p-8">
        ...
        <div class="card overflow-hidden">
            <div class="px-6 py-4 font-black text-sm text-primary" style="border-bottom:1px solid var(--border);">All Customers <span class="text-secondary font-semibold">(3)</span></div>
        ...
        <div class="card overflow-hidden mb-6">
            <div class="card-header">
                ...
                <div>
                    ...
                    <div class="text-xs text-secondary font-semibold mt-0.5">Upload up to 5 rotating banners for the homepage slider</div>
                </div>
                <button onclick="document.getElementById('bannerFileInput').click()" class="btn-primary ml-auto text-sm">+ Upload Banner</button>
                <input type="file" id="bannerFileInput" accept="image/*" class="hidden" onchange="addBanner(event)">
            </div>
            <div class="p-6">
                <div class="grid grid-cols-3 gap-4" id="bannersGrid">
                    ...
                    <div class="banner-dropzone flex flex-col items-center justify-center gap-2 text-secondary" onclick="document.getElementById('bannerFileInput').click()" style="height:140px;">
            ...
            <div class="card overflow-hidden">
                ...
                <div class="p-6 flex flex-col gap-4">
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <label class="flex items-center justify-between"><span class="text-sm font-bold text-primary">📱 Khalti</span><label class="switch"><input type="checkbox" checked><span class="slider"></span></label></label>
                        <label class="flex items-center justify-between"><span class="text-sm font-bold text-primary">🏦 Bank Transfer</span><label class="switch"><input type="checkbox"><span class="slider"></span></label></label>
                    </div>
    ...
    <div id="page-delivery" class="page-section p-8">
        <div class="flex items-center justify-between mb-7">
            <div>
                ...
                <div class="text-[13px] text-secondary font-semibold mt-1">Configure delivery areas, fees, and time slots</div>
            </div>
    ...
    <div id="page-add-product" class="page-section flex-1 p-8 pb-12">
        <div class="flex items-center justify-between mb-7">
            <div>
                ...
                <div class="text-[13px] text-secondary font-semibold mt-1">Fill in the details below to list a new product</div>
            </div>
        ...
        <div class="grid gap-6" style="grid-template-columns: 1fr 360px; align-items: start;">
            <div class="flex flex-col gap-5">
                ...
                <div class="card overflow-hidden">
                    <div class="card-header">
                        <div class="card-header-icon">📝</div>
                        <div><div class="font-black text-[15px] text-primary">Basic Information</div><div class="text-xs text-secondary font-semibold mt-0.5">Name, description and category</div></div>
                    </div>
                    <div class="p-6">
                        <div class="mb-5">
                            <label class="block text-[13px] font-extrabold text-primary mb-1.5">Product Name <span class="text-red-500">*</span></label>
                            <input name="name" type="text" id="productName" placeholder="e.g. Fresh Organic Tomatoes" oninput="updatePreview()" maxlength="60" class="form-input">
                            <div class="text-[11px] text-secondary font-bold text-right mt-1" id="nameCounter">0 / 60</div>
                            <div class="text-[11px] text-red-500 font-bold mt-1 hidden" id="nameError">Product name is required</div>
                        </div>
                        <div class="mb-5">
                            <label class="block text-[13px] font-extrabold text-primary mb-1.5">Description <span class="text-[11px] font-semibold text-secondary ml-1.5">Optional</span></label>
                            <textarea name="description" id="productDesc" placeholder="Describe the product…" maxlength="300" oninput="updateDescCounter()" rows="3" class="form-input" style="resize:vertical;"></textarea>
                            <div class="text-[11px] text-secondary font-bold text-right mt-1" id="descCounter">0 / 300</div>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-[13px] font-extrabold text-primary mb-1.5">Category <span class="text-red-500">*</span></label>
                                <select name="category" id="productCategory" onchange="updatePreview()" class="form-input">
                            ...
                            <div>
                                <label class="block text-[13px] font-extrabold text-primary mb-1.5">Unit / Weight <span class="text-red-500">*</span></label>
                                <input type="text" name="unit" id="productUnit" placeholder="e.g. 500g, 1L, 6 pcs" oninput="updatePreview()" class="form-input">
                ...
                <div class="card overflow-hidden">
                    <div class="card-header">
                        <div class="card-header-icon">💰</div>
                        <div><div class="font-black text-[15px] text-primary">Pricing</div><div class="text-xs text-secondary font-semibold mt-0.5">Set the selling and compare-at price</div></div>
                    </div>
                    <div class="p-6">
                        <div class="grid grid-cols-2 gap-4 mb-2">
                            <div>
                                <label class="block text-[13px] font-extrabold text-primary mb-1.5">Selling Price <span class="text-red-500">*</span></label>
                                <div class="relative">
                                    <div class="input-prefix">RS</div>
                                    <input type="number" name="price" id="productPrice" placeholder="0" min="0" oninput="updatePreview()" class="form-input" style="padding-left:52px;">
                                </div>
                                <div class="text-[11px] text-red-500 font-bold mt-1 hidden" id="priceError">Price is required</div>
                            </div>
                            <div>
                                <label class="block text-[13px] font-extrabold text-primary mb-1.5">Compare-at Price <span class="text-[11px] font-semibold text-secondary ml-1">Optional</span></label>
                                <div class="relative">
                                    <div class="input-prefix">RS</div>
                                    <input type="number" id="comparePrice" placeholder="0" min="0" oninput="updatePreview()" class="form-input" style="padding-left:52px;">
                                </div>
                            </div>
                        </div>
                        <div id="discountBadge" class="hidden mt-1"><span style="background:#e8f5e9;color:#2e7d32;" class="text-xs font-extrabold px-2.5 py-1 rounded-lg">🏷️ <span id="discountPct"></span>% OFF shown to customers</span></div>
                    </div>
                </div>
                ...
                <div class="card overflow-hidden">
                    <div class="card-header">
                        <div class="card-header-icon">🖼️</div>
                        <div><div class="font-black text-[15px] text-primary">Product Image</div><div class="text-xs text-secondary font-semibold mt-0.5">JPG, PNG or WEBP — recommended 800×800px</div></div>
                    </div>
                    <div class="p-6">
                        <div id="uploadZone" class="upload-zone border-2 border-dashed rounded-2xl p-7 text-center relative" style="border-color:var(--border);background:var(--input-bg);" onclick="document.getElementById('imageInput').click()">
                            <input type="file" name="image" id="imageInput" accept="image/*" onchange="handleImageUpload(event)" class="hidden">
                            ...
                            <div id="uploadPreviewWrap" class="hidden relative">
                                ...
                                <div class="upload-preview-overlay absolute inset-0 bg-black/45 rounded-xl items-center justify-center text-white font-extrabold text-[13px] gap-1.5">📷 Change Image</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex flex-col gap-5">
                ...
                <div class="card overflow-hidden">
                    <div class="card-header">
                        <div class="card-header-icon">👁️</div>
                        <div><div class="font-black text-[15px] text-primary">Live Preview</div><div class="text-xs text-secondary font-semibold mt-0.5">How it looks in the store</div></div>
                    </div>
                    <div class="p-6">
                        <div class="border-theme border rounded-2xl overflow-hidden max-w-[200px] mx-auto">
                            <div class="h-[130px] flex items-center justify-center relative overflow-hidden" style="background:#e8f5ee;">
                            ...
                            <div class="p-2.5 pb-3">
                                <div id="previewName" class="font-extrabold text-[13px] text-primary mb-0.5 min-h-[18px]">Product name</div>
                                <div id="previewUnit" class="text-[11px] text-secondary font-semibold mb-2">Unit</div>
                                <div class="flex items-center justify-between">
                                    <div id="previewPrice" class="font-black text-[15px] text-primary">RS —</div>
                ...
                <div class="card overflow-hidden">
                    ...
                    <div class="p-5">
                        <div class="flex flex-wrap gap-2">
                            ...
                            <label for="tagFresh" class="tag-label" style="border-color:#c8e6c9;color:#2e7d32;">🌿 Fresh</label>
                            <input type="radio" name="tag" id="tagNew" value="New" class="tag-option" onchange="updateTag()">
                            <label for="tagNew" class="tag-label" style="border-color:#bbdefb;color:#0d47a1;">✨ New</label>
                            <input type="radio" name="tag" id="tagOrganic" value="Organic" class="tag-option" onchange="updateTag()">
                            <label for="tagOrganic" class="tag-label" style="border-color:#e1bee7;color:#6a1b9a;">🌱 Organic</label>
                            <input type="radio" name="tag" id="tagBest" value="Best Seller" class="tag-option" onchange="updateTag()">
                            <label for="tagBest" class="tag-label" style="border-color:#ffe0b2;color:#e65100;">🔥 Best Seller</label>
                            <input type="radio" name="tag" id="tagPopular" value="Popular" class="tag-option" onchange="updateTag()">
                            <label for="tagPopular" class="tag-label" style="border-color:#fce4ec;color:#880e4f;">⭐ Popular</label>
                            <input type="radio" name="tag" id="tagHealthy" value="Healthy" class="tag-option" onchange="updateTag()">
                            <label for="tagHealthy" class="tag-label" style="border-color:#b2dfdb;color:#004d40;">💚 Healthy</label>
                        </div>
                    </div>
                </div>
                ...
                <button type="button" onclick="submitForm()" class="btn-primary w-full py-3.5 text-[15px] flex items-center justify-center gap-2">✓ Publish Product</button>
                <button type="button" onclick="resetForm()" class="btn-ghost w-full py-3 text-[13px]">✕ Discard Changes</button>
            </div>
        </div>
    </div>
...
<div id="orderModal" class="modal-backdrop">
    <div class="modal-box p-0 w-[500px]">
        <div class="flex items-center justify-between px-6 py-4" style="border-bottom:1px solid var(--border);">
            ...
            <button onclick="document.getElementById('orderModal').classList.remove('open')" class="rounded-full w-8 h-8 text-lg cursor-pointer font-nunito text-primary border-none" style="background:var(--input-bg);">✕</button>
        </div>
...
<div id="customerModal" class="modal-backdrop">
    <div class="modal-box p-0 w-[480px]">
        <div class="flex items-center justify-between px-6 py-4" style="border-bottom:1px solid var(--border);">
            ...
            <button onclick="document.getElementById('customerModal').classList.remove('open')" class="rounded-full w-8 h-8 text-lg cursor-pointer font-nunito text-primary border-none" style="background:var(--input-bg);">✕</button>
        </div>
...
<!-- Toast -->
<div id="toast" class="toast fixed bottom-8 right-8 px-5 py-3.5 rounded-2xl text-sm font-bold flex items-center gap-2.5 shadow-2xl z-[999]" style="background:#1a1a2e;color:#fff;">
...
const orders = [
    ...
    {id:'ORD-003',customer:'Hari Pradhan',email:'hari@example.com',phone:'9861234567',address:'Kupondole, Lalitpur',items:'Mixed Nuts × 1, Dark Chocolate × 2',total:365,status:'Delivered',date:'2026-03-28',payment:'Khalti'},
    {id:'ORD-004',customer:'Anita KC',email:'anita@example.com',phone:'9871234567',address:'Jawalakhel, Lalitpur',items:'Almond Milk × 1, Orange Juice × 1',total:270,status:'Pending',date:'2026-03-28',payment:'Cash on Delivery'},
    {id:'ORD-005',customer:'Bijay Lama',email:'bijay@example.com',phone:'9881234567',address:'Durbar Square, Bhaktapur',items:'Sourdough Bread × 3',total:255,status:'Processing',date:'2026-03-27',payment:'eSewa'},
];

const customers = [
    {id:'USR-001',name:'Ram Sharma',email:'ram@example.com',phone:'9841234567',address:'House No. 12, Thamel Marg, Kathmandu 44600',gender:'Male',dob:'1990-05-14',joined:'2025-11-03',orders:8,total:2340,status:'Active'},
    {id:'USR-002',name:'Sita Thapa',email:'sita@example.com',phone:'9851234567',address:'Flat 3B, Lazimpat Height, Kathmandu 44600',gender:'Female',dob:'1994-08-22',joined:'2025-12-18',orders:5,total:1200,status:'Active'},
...
function renderProducts(list) {
    ...
    if (!tbody) return;
    if (!list || list.length === 0) { tbody.innerHTML = `<div class="px-6 py-12 text-center text-secondary font-bold">No products found</div>`; return; }
...
function renderDiscounts() {
    ...
    if (discounts.length === 0) {
        body.innerHTML = `<div class="p-12 text-center text-secondary"><div class="text-5xl mb-3">🎟️</div><div class="font-bold text-base">No active discount codes</div><button onclick="openDiscountModal()" class="btn-primary mt-4">+ Create Discount</button></div>`;
        return;
    }
...
function toggleFeatured(id) {
    ...
    else { showToast('⚠️ Max 8 featured products'); return; }
    renderFeatured();
...
function updateTag() {
    ...
    if (tag && tagMeta[tag]) { previewTag.classList.remove('hidden'); previewTag.textContent = tag; previewTag.style.background = tagMeta[tag].bg; previewTag.style.color = tagMeta[tag].color; }
...
function updateStock() {
    ...
    if (document.getElementById('stockQty').value !== '') {
        display.classList.remove('hidden'); display.style.display = 'flex';
    ...
    } else { display.classList.add('hidden'); display.style.display = ''; }
}
...
function validate() {
    let ok = true;
    [{id:'productName',err:'nameError',check:v=>v.trim()!==''},{id:'productCategory',err:'catError',check:v=>v!==''},{id:'productUnit',err:'unitError',check:v=>v.trim()!==''},{id:'productPrice',err:'priceError',check:v=>v!==''&&parseFloat(v)>=0}].forEach(f => {
        const el = document.getElementById(f.id), err = document.getElementById(f.err);
        if (!f.check(el.value)) { el.classList.add('error'); err.classList.remove('hidden'); ok = false; }
        else { el.classList.remove('error'); err.classList.add('hidden'); }
    });
    return ok;
}
function submitForm() {
